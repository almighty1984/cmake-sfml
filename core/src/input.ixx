...
export namespace input {
    ...
    class Set {
        static inline std::vector<Object*> m_input_objects;
    ...
    public:
        ...
        static bool   is_valid(size_t i) { return (i < m_input_objects.size() && m_input_objects.at(i)) ? true : false; }
        static Object* at(size_t i) { return (i < m_input_objects.size()) ? m_input_objects.at(i) : nullptr; }
        ...
        static void handle_events(Window& w) {
            while (const std::optional event = w.poll_event()) {
                ...
                if (event->is<sf::Event::Closed>()) {
                    ...
                    Button button = (Button)mouse_pressed->button;
                    ...
                    Button b = (Button)mouse_released->button;
        ...
        static i32c make() {
            ...
            if (!m_unused_ids.empty()) {
                ...
                input->id = m_input_objects.size();
                m_input_objects.push_back(nullptr);
            }
            if (input->id >= 0 && input->id < m_input_objects.size() && m_input_objects.at(input->id)) {
                delete m_input_objects.at(input->id);
            }
            m_input_objects.at(input->id) = input;
            return input->id;
        }
        static bool erase(size_t i) {
            if (!(i < m_input_objects.size()) || !m_input_objects.at(i)) {
                Console::log("input::Set::erase ", i, " can't do it\n");
                return false;
            }
            delete m_input_objects.at(i);
            m_input_objects.at(i) = nullptr;
            m_unused_ids.push_back(i);
            return true;
        }
        static void clear() {
            for (size_t i = 0; i < m_input_objects.size(); ++i) {
