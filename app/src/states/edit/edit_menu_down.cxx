...
namespace state {
    void Edit::init_brush(u8 size) {
        ...
        for (const auto& i : m_mouse_sprite_ids) sprite::Set::erase(i);        
        ...
        for (f32 x = start.x; x < end.x; x += 16.0f) {
            for (f32 y = start.y; y < end.y; y += 16.0f) {
                ...
                sprite::Set::at(m_mouse_sprite_ids.back())->id = m_mouse_sprite_ids.back();
                sprite::Set::at(m_mouse_sprite_ids.back())->transform_id = m_mouse_transform_id;
                sprite::Set::at(m_mouse_sprite_ids.back())->layer = 13;
                sprite::Set::at(m_mouse_sprite_ids.back())->offset.x = x;
                sprite::Set::at(m_mouse_sprite_ids.back())->offset.y = y;                
            }
        }
    }
    void Edit::handle_menu_down() {
        ...
        if (m_is_showing_tile_set) {            
            if (m_tile_set == 255 && position.x == 272.0f && position.y == 160.0f) {
            ...
            if (position.x == 272.0f && position.y == 160.0f) {
