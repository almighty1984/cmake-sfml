...
namespace menu {
    ...
    struct List {
        ...
        ~List() {
            transform::Manager::erase(transform_id);
            sprite::Manager::erase(bg_sprite_id);
            items.clear();
        }

        void draw(std::unique_ptr<Window>& window) {
            window->draw(sprite::Manager::get(bg_sprite_id));
            for (auto& i : items) {
                if (i) i->draw(window);
            }
        }
...
export namespace state {
    ...
    private:        
        I32 m_mouse_transform_id           = -1,
            ...
            m_tile_set_scroll_transform_id = -1;
        ...
        sprite::Object* level_sprite_at_offset(cU8 layer, cVec2F offset);

        undo::Act place_sprite_on_level(cU8 layer, cU8 tile_set, cRectI source_rect, cVec2F offset);
        undo::Act erase_sprite_on_level(cU8 layer, cVec2F offset);
        ...
        void draw(std::unique_ptr<Window>& window, cU8 layer) override {
            ...
            if (sprite::Manager::get(m_menu_down_bg_sprite_id)->layer == layer)             window->draw(sprite::Manager::get(m_menu_down_bg_sprite_id));
            if (sprite::Manager::get(m_menu_right_bg_sprite_id)->layer == layer)            window->draw(sprite::Manager::get(m_menu_right_bg_sprite_id));
            if (sprite::Manager::get(m_position_on_grid_map_sprite_id)->layer == layer)     window->draw(sprite::Manager::get(m_position_on_grid_map_sprite_id));
            if (sprite::Manager::get(m_save_sprite_id)->layer == layer)                     window->draw(sprite::Manager::get(m_save_sprite_id));
            if (sprite::Manager::get(m_tile_set_bg_sprite_id)->layer == layer)              window->draw(sprite::Manager::get(m_tile_set_bg_sprite_id));
            if (sprite::Manager::get(m_tile_set_sprite_id)->layer == layer)                 window->draw(sprite::Manager::get(m_tile_set_sprite_id));
            if (sprite::Manager::get(m_current_tile_sprite_id)->layer == layer)             window->draw(sprite::Manager::get(m_current_tile_sprite_id));
            if (sprite::Manager::get(m_grid_icon_sprite_id)->layer == layer)                window->draw(sprite::Manager::get(m_grid_icon_sprite_id));
            if (sprite::Manager::get(m_active_layer_sprite_id)->layer == layer)             window->draw(sprite::Manager::get(m_active_layer_sprite_id));
            if (sprite::Manager::get(m_text_bar_bg_sprite_id)->layer == layer)              window->draw(sprite::Manager::get(m_text_bar_bg_sprite_id));
            if (sprite::Manager::get(m_text_current_tile_set_bg_sprite_id)->layer == layer) window->draw(sprite::Manager::get(m_text_current_tile_set_bg_sprite_id));

            for (auto& i : m_is_hidden_layer_sprite_ids) {
                if (sprite::Manager::get(i)->layer == layer) window->draw(sprite::Manager::get(i));
            }
            ...
            for (auto& i : m_level_sprite_ids) {
                if (sprite::Manager::get(i)->layer == layer) {
                    window->draw(sprite::Manager::get(i));
                }
            }
