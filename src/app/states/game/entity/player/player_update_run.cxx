...
void player::Object::update_run() {
    ...
    if (m_time_left_bouncing > 0) {
        ...
        if (m_time_left_bouncing == 0 && m_is_on_ground) {
            transform()->velocity_limit = m_ground_velocity_limit;
        }
    }
    ...
    //Console::log("m_aabb_ids size: ", m_aabb_ids.size(), "\n");
    /*for (auto& i : m_aabb_ids) {
        Console::log("name: ", aabb::name_as_string(aabb::Set::at(i)->name), "\n");
        if (aabb::Set::at(i)->name == aabb::Name::hit_ground) {
            Console::log("hello\n");
            aabb::Set::at(i)->is_inactive(false);
        }
    }*/
    if (m_time_left_hitting_ground > 0) {
        ...
        if (m_time_left_hitting_ground == m_time_to_hit_ground - 0) {            
            /*for (auto& i : m_hit_ground_aabb_ids) {
                aabb::Set::at(i)->is_inactive(false);
            }*/
        } else {
            /*for (auto& i : m_hit_ground_aabb_ids) {
    ...
    if (m_time_left_melee > 0) {
        --m_time_left_melee;
        for (auto& i : m_aabb_ids) {
            if (sprite()->is_leftward && aabb::Set::at(i)->name == aabb::Name::melee_L) {
                aabb::Set::at(i)->is_inactive(m_time_left_melee == 0);
            } else if (!sprite()->is_leftward && aabb::Set::at(i)->name == aabb::Name::melee_R) {
                aabb::Set::at(i)->is_inactive(m_time_left_melee == 0);
            }
        }
        /*for (auto& i : m_melee_left_aabb_ids) {
            aabb::Set::at(i)->is_inactive(!sprite()->is_leftward || m_time_left_melee == 0);
        }
        for (auto& i : m_melee_right_aabb_ids) {
            aabb::Set::at(i)->is_inactive(sprite()->is_leftward || m_time_left_melee == 0);
        }*/
    ...
    if (m_is_on_ground || m_is_on_ledge) {
        ...
        if (m_is_carrying) {
            transform()->velocity_limit.x *= 0.9f;
        }
    }


    if (transform()->velocity.y > transform()->acceleration.y + (m_is_on_slope ? transform()->velocity_limit.y : 1.0f)) {
    ...
    if (m_is_on_slope && transform()->velocity.y >= transform()->velocity_limit.y) {
        m_is_on_slope = false;
    }
    ...
    if (m_time_left_interacting > 0) {
        ...
        for (auto& i : m_aabb_ids) {
            if (!aabb::Set::at(i)) continue;
            if (aabb::Set::at(i)->name == aabb::Name::interact) {
                aabb::Set::at(i)->is_inactive(false);
            }
        }
        if (m_time_left_interacting == 0) {
    ...
    if (m_next_state == entity::State::swim && m_time_left_in_state == 0) {
        ...
        //for (auto& i : m_run_aabb_ids)  aabb::Set::at(i)->is_inactive(true);
        //for (auto& i : m_swim_aabb_ids) aabb::Set::at(i)->is_inactive(false);
    }
