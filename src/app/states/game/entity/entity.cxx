...
//std::mutex config_load_mutex;

aabb::Object* entity::Object::aabb_object(aabb::cName name) {
    for (auto& i : m_aabb_ids) {
        if (aabb::Manager::get(i)->name == name) {
            return aabb::Manager::get(i);
        }
    }
    return nullptr;
}
void entity::Object::draw_aabb(std::unique_ptr<Window>& window) {
    for (auto& i : m_aabb_ids) {
        aabb::Manager::get(i)->draw(window);
    }
}
...
entity::Object::~Object() {
    ...
    for (auto& i : m_aabb_ids) {
        aabb::Manager::erase(i);
    }
    m_aabb_ids.clear();
    input_objects.clear();

    anim::Manager::erase(m_bounce_anim_id);    
    anim::Manager::erase(m_dead_anim_id);
    anim::Manager::erase(m_hurt_anim_id);
    anim::Manager::erase(m_idle_anim_id);
    anim::Manager::erase(m_jump_anim_id);
    anim::Manager::erase(m_melee_anim_id);
    anim::Manager::erase(m_upended_anim_id);

    health::Manager::erase(m_health_id);

    sound::Manager::erase(m_block_sound_id);
    sound::Manager::erase(m_bounce_sound_id);
    sound::Manager::erase(m_bump_head_sound_id);
    sound::Manager::erase(m_dead_sound_id);
    sound::Manager::erase(m_hurt_sound_id);
    sound::Manager::erase(m_melee_sound_id);
    
    sprite::Manager::erase(m_sprite_id);
    
    //if (m_start_offset == Vec2F{ 0.0F, 0.0F}) {
    if (m_transform_id != level_transform_id && m_transform_id != -1) {
...
bool entity::Object::load_config(const std::filesystem::path& path) {
    ...
    if (text.find("{", transform_label) != std::string::npos) {
        ...
        //transform::Manager::get(m_transform_id)->position = m_start_offset;
        ...
        if (transform_close != std::string::npos) {
            ...
            if (velocity_limit_start < end_line && transform::Manager::get(m_transform_id)) {
                ...
                if (velocity_limit_open < end_line && velocity_limit_close < end_line) {
                    ...
                    if (comma_0 < end_line) {
                        transform::Manager::get(m_transform_id)->max_velocity.x = std::stof(text.substr(value_0, comma_0 - value_0));
                    }
                    ...
                    transform::Manager::get(m_transform_id)->max_velocity.y = std::stof(text.substr(value_1, value_1_end - value_1));
                }
            }
            ...
            if (acceleration_start < end_line && transform::Manager::get(m_transform_id)) {
                ...
                if (acceleration_open < end_line && acceleration_close < end_line) {
                    ...
                    const size_t comma_0 = text.find(",", value_0);
                    if (comma_0 < end_line) {
                        transform::Manager::get(m_transform_id)->acceleration.x = std::stof(text.substr(value_0, comma_0 - value_0));
                    }
                    size_t value_1 = comma_0 + 1;
                    while (text.at(value_1) == '	' || text.at(value_1) == ' ') ++value_1;
                    ...
                    transform::Manager::get(m_transform_id)->acceleration.y = std::stof(text.substr(value_1, value_1_end - value_1));
                }
            }
            ...
            if (deceleration_start < end_line && transform::Manager::get(m_transform_id)) {
                ...
                if (deceleration_open < end_line && deceleration_close < end_line) {
                    ...
                    const size_t comma_0 = text.find(",", value_0);
                    if (comma_0 < end_line) {
                        transform::Manager::get(m_transform_id)->deceleration.x = std::stof(text.substr(value_0, comma_0 - value_0));
                    }
                    size_t value_1 = comma_0 + 1;
                    while (text.at(value_1) == '	' || text.at(value_1) == ' ') ++value_1;
                    ...
                    transform::Manager::get(m_transform_id)->deceleration.y = std::stof(text.substr(value_1, value_1_end - value_1));
                }
            }
    ...
    if (text.find("{", aabb_set_label) != std::string::npos) {
        ...
        if (aabb_set_close != std::string::npos) {
            ...
            while (aabb_open < aabb_set_close) {
                ...
                cI32 aabb_id = aabb::Manager::make(m_transform_id, { m_start_offset.x + aabb_rect.x,
                                                                 m_start_offset.y + aabb_rect.y,
                                                                 aabb_rect.w,
                                                                 aabb_rect.h });
                aabb::Manager::get(aabb_id)->id = aabb_id;
                aabb::Manager::get(aabb_id)->name = aabb_name;
                aabb::Manager::get(aabb_id)->owner = this;
                aabb::Manager::get(aabb_id)->update();
        ...
        if (aabb_colors_close != std::string::npos) {
            ...
            while (color_open < aabb_colors_close) {
                ...
                for (auto& aabb_id : m_aabb_ids) {
                    if (aabb::Manager::get(aabb_id) && aabb::Manager::get(aabb_id)->name == aabb_name) {
                        aabb::Manager::get(aabb_id)->color(aabb_color);
                        aabb::Manager::get(aabb_id)->start_color(aabb_color);
                    }
                }
    ...
    if (text.find("{", sounds_label) != std::string::npos) {
        ...
        if (sounds_close != std::string::npos) {
            ...
            while (1) {
                ...
                if      (sound_label == "block")       m_block_sound_id       = sound::Manager::make(sound_path);
                else if (sound_label == "bounce")      m_bounce_sound_id      = sound::Manager::make(sound_path);
                else if (sound_label == "bump_head")   m_bump_head_sound_id   = sound::Manager::make(sound_path);
                else if (sound_label == "dead")        m_dead_sound_id        = sound::Manager::make(sound_path);
                else if (sound_label == "hurt")        m_hurt_sound_id        = sound::Manager::make(sound_path);
                else if (sound_label == "melee")       m_melee_sound_id       = sound::Manager::make(sound_path);
                else if (sound_label == "water_enter") m_water_enter_sound_id = sound::Manager::make(sound_path);
                else if (sound_label == "water_exit")  m_water_exit_sound_id  = sound::Manager::make(sound_path);
                label_start = end_line;
            }
        }
    }
sound_block_end:
    ...
    if (text.find("{", text.find("Sprite", 0)) != std::string::npos) {
        ...
        if (sprite_close != std::string::npos) {
            ...
            m_sprite_id = sprite::Manager::make(sprite_texture_path);

            sprite::Manager::get(m_sprite_id)->layer = m_start_layer;
            sprite::Manager::get(m_sprite_id)->is_debug = sprite_is_debug;
            sprite::Manager::get(m_sprite_id)->is_hidden = sprite_is_hidden;
            sprite::Manager::get(m_sprite_id)->is_leftward = sprite_is_leftward;
            sprite::Manager::get(m_sprite_id)->is_upended = sprite_is_upended;
            sprite::Manager::get(m_sprite_id)->source_rect = sprite_source_rect;
                
            if (sprite_origin == Vec2F{ 0.0f, 0.0f }) {
                sprite::Manager::get(m_sprite_id)->origin = { sprite_source_rect.w / 2.0F, sprite_source_rect.h / 2.0F };
            } else {
                sprite::Manager::get(m_sprite_id)->origin = sprite_origin;
            }

            sprite::Manager::get(m_sprite_id)->offset = m_start_offset + sprite_offset; // Add to already set level offset

            sprite::Manager::get(m_sprite_id)->transform_id = m_transform_id;
                ...
                while (1) {
                    ...
                    if (current_open < end_line && current_close < end_line) {
                        ...
                        if (current_label_str == "bounce") {
                            m_bounce_anim_id = anim::Manager::make();   m_current_anim_id = m_bounce_anim_id;
                        } else if (current_label_str == "dead") {
                            m_dead_anim_id = anim::Manager::make();     m_current_anim_id = m_dead_anim_id;
                        } else if (current_label_str == "hurt") {
                            m_hurt_anim_id = anim::Manager::make();     m_current_anim_id = m_hurt_anim_id;
                        } else if (current_label_str == "idle") {
                            m_idle_anim_id = anim::Manager::make();     m_current_anim_id = m_idle_anim_id;
                        } else if (current_label_str == "jump") {
                            m_jump_anim_id = anim::Manager::make();     m_current_anim_id = m_jump_anim_id;
                        } else if (current_label_str == "melee") {
                            m_melee_anim_id = anim::Manager::make();    m_current_anim_id = m_melee_anim_id;
                        } else if (current_label_str == "stunned") {
                            m_stunned_anim_id = anim::Manager::make();  m_current_anim_id = m_stunned_anim_id;
                        } else if (current_label_str == "upended") {
                            m_upended_anim_id = anim::Manager::make();  m_current_anim_id = m_upended_anim_id;
                        }
...
sprite_and_animations_block_end:
    ...
    m_health_id = health::Manager::make(m_transform_id);
    if (text.find("{", health_label) != std::string::npos && health::Manager::get(m_health_id)) {
        ...
        if (health_close != std::string::npos) {
            ...
            if (amount_label < health_close) {
                ...
                if (value_start < end_line) {
                    ...
                    health::Manager::get(m_health_id)->max = amount;
                    health::Manager::get(m_health_id)->amount = amount;
                }
            }
            ...
            if (regen_label < health_close) {
                ...
                if (value_start < end_line) {
                    ...
                    health::Manager::get(m_health_id)->regen = regen;
                }
            }
            ...
            if (offset_start < end_line) {
                ...
                if (offset_open < end_line && offset_close < end_line) {
                    ...
                    const size_t comma_0 = text.find(",", value_0);
                    if (comma_0 < end_line) {
                        health::Manager::get(m_health_id)->offset.x = std::stof(text.substr(value_0, comma_0 - value_0));
                    }
                    size_t value_1 = comma_0 + 1;
                    while (text.at(value_1) == '	' || text.at(value_1) == ' ') ++value_1;
                    size_t value_1_end = offset_close;
                    ...
                    health::Manager::get(m_health_id)->offset.y = std::stof(text.substr(value_1, value_1_end - value_1));
                }
            }
        }
    }
