...
public:
    ~LevelConfig() {
        music::Manager::erase(m_music_id);
        music::Manager::clear();
    }
    ...
    static bool stop_music() {
        if (!music::Manager::get(m_music_id)) return false;
        music::Manager::get(m_music_id)->stop();
        return true;
    }
            ...
            if (music_label < level_close) {
                ...
                if (music_start < end_line) {
                    ...
                    if (m_level_music_path != m_prev_level_music_path) {
                        ...
                        m_music_id = music::Manager::make(m_level_music_path);

                        music::Manager::get(m_music_id)->is_looped(true);
                        music::Manager::get(m_music_id)->volume(5.0F);
                        music::Manager::get(m_music_id)->play();
                    }
                }
            }
