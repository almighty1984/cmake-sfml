...
public:
    ...
    static bool stop_music() {
        if (!music::Set::at(m_music_id)) return false;
        music::Set::at(m_music_id)->stop();
        return true;
    }
    ...
    /*static std::filesystem::path trigger_path(u8c number) {
    ...
    static void load(const std::filesystem::path& path) {
        ...
        if (level_close != std::string::npos) {
            ...
            if (trigger_label < level_close) {
                ...
                u8c number = std::stoi(text.substr(number_start, number_end - number_start));
